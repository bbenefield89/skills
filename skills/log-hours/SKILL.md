...
- **Azure DevOps** — PRs the user created/updated/reviewed that day. `@me` does **not** resolve in `az repos pr list` — use the email: `az repos pr list --creator <email>` and `--reviewer <email>`. Query **all** states — `--status active`, `--status completed`, and `--status abandoned` — then filter to activity on the target date (a PR completed or abandoned that day is signal too, not just active ones).
- **GitHub** — `gh search prs --author @me` and `--review-requested @me`, covering **open, merged, and closed** PRs updated on the target date (not just `--state open`); use `--updated <date>` or filter results to the date. Commits come via the multi-repo git scan above.
...
2. **Dev / review** → estimate per ticket from signal density (commits across all repos, PR activity in any state, transitions, comments, plus any Confluence/email attribution). Each row carries a **one-line rationale** naming its evidence (include repo names for cross-repo commits).
